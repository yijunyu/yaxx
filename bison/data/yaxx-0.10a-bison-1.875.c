...
#define YYYAXX_DTD "yaxx.dtd"
       ...
       /// version 
       fprintf(stdot,"<?xml version=\"1.0\"?>\n");
       /// DTD reference
       fprintf(stdot,"<?xml-stylesheet type=\"text/xsl\" href=\"yaxx.xsl\"?>");
       fprintf(stdot,"<!DOCTYPE %s SYSTEM \"" YYYAXX_DTD "\">\n", buf);
       /// inserting name space before the XML data
       fprintf(stdot,"<yaxx:%s xmlns:yaxx=\"urn:YAcc-Xml-eXtension\"%s\n", buf,
            yyxs[1]+i+6); //ffprintf(stdot,f, "%s\n", yyxs[1]);
       /// Generating the document type definition (DTD) 
       stdot = fopen(YYYAXX_DTD , "w");
       ...
       for (r = 2; r < sizeof(yyr1)/sizeof(unsigned short); r++) 
       {
        ...
        if (j != old_rule) {
        	if (old_rule!=0)
        		fprintf(stdot,")>\n");
	        fprintf(stdot,"<!ELEMENT %s (", YYTNAME(yyr1[r]));
        } else
	    	fprintf(stdot," | ");
        if (yyr2[r]==0) 
           fprintf(stdot,"EMPTY");
            ...
            for (i = yyprhs[r]; yyrhs[i] > 0 ; i++)
       		...
       		if (multiple) fprintf(stdot,"(");
            for (i = yyprhs[r]; yyrhs[i] > 0 ; i++) {
               if (i!=yyprhs[r])
                   fprintf(stdot,",");
               fprintf(stdot,"%s", YYTNAME(yyrhs[i]));
            }
       		if (multiple) fprintf(stdot,")");
         }
         old_rule = j;
       ...
       {
            fprintf(stdot,"<!ELEMENT %s (#PCDATA)>\n", YYTNAME(r));
       }
...
]b4_location_if([   &yyls1, yysize * sizeof (*yylsp),])[ &yystacksize);
    yyxs = yyxs1;
