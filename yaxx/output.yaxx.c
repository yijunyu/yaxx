...
/* The parser tables consist of these tables.
   ...
   * yystos[s] = the symbol number of the symbol that leads to state s.
   ...
   yytypes = a vector filled with types for selecting a field
   from the YYSTYPE union. The information can be obtained by
   yy_token_type function.
   The Input file has to define a function yy_token_value to
   convert the YYSTYPE to string according to the type info.
...
output_rule_data (void)
{
  ...
  if (!token_table_flag && !no_parser_flag)
    obstack_sgrow (&table_obstack,
			 "\n#if (YYDEBUG) || (YYXML) || defined YYERROR_VERBOSE\n\n");
  obstack_sgrow (&table_obstack, "\
...
//----------------------------------------------------------
  obstack_fgrow2 (&table_obstack,
      "#define YYTOKENTYPE(x) ((unsigned)(x) <= %d ? yytypes[x] : %s)\
\n\
\n",
		  max_user_token_number, "0");

  obstack_sgrow (&table_obstack,
	   "static const char *const yytypes[] =\n{\n  ");
